...
class Robot(Node):
    ...
    def process_item(self):
        ...
        should_process_item = self.should_process_item(item_weight, destination, item['id'])
        if should_process_item[0]:
            # we need to update the values right after the decision so it affects next items/decisions
            self.__current_battery -= self.get_estimated_battery_to_process_item(self.__sector_data[self.get_name()],
                                                                                 destination, item_weight)
            ...
            self.__items_to_be_picked_up.put((distance_to_destination, PriorityEntry(message.timestamp, item)),
                                             block=True)
    ...
    def real_process_item(self):
        ...
        self.get_logger().info(
            "Bringing item with id {} to delivery station at {}".format(item['id'], self.__delivery_stations[0]))
        ...
        time.sleep(self.estimate_time_to_move(from_current_position, self.__delivery_stations[0], self.__speed,
                                              self.__cell_length))
        ...
        self.get_logger().info(
            "Battery after processing item with id {}: {}".format(item['id'], self.__current_battery))
        # TODO: currently the robot stays at the delivery station until a new task is picked up

        metrics_message = String()
        metrics_message.data = json.dumps({'robot_id': self.get_name(), 'item_id': item['id'],
                                           ...
                                           'robot_position': from_position, 'item_position': destination,
                                           'delivery_station': self.__delivery_stations[0],
                                           'total_distance': from_position.distance(
                                               destination) + from_current_position.distance(
                                               self.__delivery_stations[0]),
                                           'processing_lasted_in_seconds': processing_end_seconds - processing_start_seconds,
                                           'number_of_items': 1, 'battery_used_for_task': self.__current_battery},
                                          default=vars)
        self.get_logger().info("Published metrics")
    ...
    def publish_robot_data(self):
        ...
        ros_message.data = json.dumps(
            {self.get_name(): {'battery': self.__current_battery, 'total_capacity': self.__total_capacity,
    ...
    def should_process_item(self, item_weight, destination, item_id):
        ...
        available_robots = [key for key, value in self.__sector_data.items() if
                            not value['charging'] and value['current_capacity'] + item_weight <= value[
                                'total_capacity'] and value['battery'] > self.get_estimated_battery_to_process_item(
                                value, destination, item_weight) + 1]
    ...
    def get_estimated_battery_to_process_item(self, value, destination, item_weight):
        additional_battery = self.get_additional_battery_cost(self.__current_capacity + item_weight,
                                                              self.__total_capacity)
        return (self.estimate_battery_capacity(value['current_position'], destination, value['battery_per_cell'],
                ...
                self.estimate_battery_capacity(value['delivery_station'], value['initial_position'],
                                               value['battery_per_cell'], additional_battery))
    ...
    def battery_handler(self):
        ...
        battery_to_go_to_charging_station = self.estimate_battery_capacity(self.__current_position,
                                                                           self.__initial_position,
                                                                           self.__battery_per_cell,
                                                                           0)
        if (not self.__charging and battery_to_go_to_charging_station +
                1 >= self.__current_battery):
            ...
            time.sleep(self.estimate_time_to_move(from_current_position, self.__initial_position, self.__speed,
                                                  self.__cell_length))
    ...
    def estimate_battery_capacity(self, source, destination, cells_per_battery_unit, additional_battery):
        return self.__normal_round(source.distance(destination) / cells_per_battery_unit) + additional_battery

    def get_additional_battery_cost(self, weight, weight_capacity):
        weight_ratio = (weight * 100) / weight_capacity
        additional_battery = 0
        if 40 < weight_ratio < 80:
            additional_battery = 1
        elif weight_ratio > 80:
            additional_battery = 2
        return additional_battery

    def estimate_time_to_move(self, source, destination, speed, cell_length):
        time_to_move_one_cell = cell_length / speed
        return self.__normal_round(source.distance(destination) * time_to_move_one_cell)
